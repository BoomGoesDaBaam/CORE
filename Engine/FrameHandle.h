	...
	void LoadScene(int scene, World* world)
	{
		if (Settings::framesOn)
		{
			...
			if (scene == 0)
			{
				...
				//comps["b_NextTurn"] = std::make_unique<Button>(Button(RectF(Vec2(650, 450), 120, 60), &resC->tC.windowsFrame[3], &resC->tC.windowsFrame[3], { 0,0 }, &resC->tC.fonts[0], nullptr, &buffer));
				...
				*/
				...
				text = fInventoryWrought->AddText(Settings::lang_craftedItem[Settings::lang] + ":", RectF(Vec2(10, 10), 50, 8), 14, &resC->tC.fonts[0], Colors::Black, "t_craft", { 1,1 }, 1);
				text->SetVisible(true);
				text = fInventoryWrought->AddText(Settings::lang_status[Settings::lang] + ":", RectF(Vec2(10, 28), 50, 8), 7, &resC->tC.fonts[0], Colors::Black, "t_craftStatus", { 1,1 }, 1);
